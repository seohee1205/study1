...
print(train_csv.isnull().sum())         # 결측치 제거 확인용
print(train_csv.info())
...
x_train, x_test, y_train, y_test = train_test_split(
    x, y, shuffle = True, train_size = 0.8, random_state = 1200
)
                                                                # 결측치 제거 후
...
def RMSE(y_test, y_predict):                # RMSE 함수 정의
    return np.sqrt(mean_squared_error(y_test, y_predict))    # return : 함수에서의 반환 , np.sqrt() : 루트
...
submission.to_csv(path + 'submit_0306_0606.csv')
